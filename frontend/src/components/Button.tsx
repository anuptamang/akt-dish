...
const buttonClasses = 'inline-flex items-center justify-center w-32 p-2 h-14 bg-red-200 text-black rounded text-center mb-2'
      ...
      props.variant === 'Link' ? 
       ...
       :
        (props.variant === 'submit' ? <button type="submit" className={`${buttonClasses} ${props.width === 'full' && 'w-full'} ${props.size === 'lg' && 'text-lg w-40'} ${props.disabled && 'pointer-events-none opacity-50'}`}>
          {props.children}
        </button> : <button className={`${buttonClasses} ${props.width === 'full' && 'w-full'} ${props.size === 'lg' && 'text-lg w-40'} ${props.disabled && 'pointer-events-none opacity-50'}`}>
          {props.children}
        </button>)
    }
    </>
